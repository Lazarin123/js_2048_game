...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    function createBoard() {
        board = [];
        for(let i = 0; i < width * width; i++)board.push(0);
        addNumber();
        addNumber();
        updateDisplay();
    }
    ...
    function addNumber() {
        const emptyCells = board.map((val, index)=>val === 0 ? index : null).filter((val)=>val !== null);
    ...
    function combine(row) {
        for(let i = 0; i < width - 1; i++)if (row[i] !== 0 && row[i] === row[i + 1]) {
            row[i] *= 2;
            row[i + 1] = 0;
            score += row[i];
            if (row[i] === 2048) showWin();
        }
        return row;
    }
    function moveLeft() {
    ...
    function moveRight() {
        ...
        for(let i = 0; i < width * width; i += width){
            ...
            if (JSON.stringify(board.slice(i, i + width)) !== JSON.stringify(newRow)) moved = true;
            board.splice(i, width, ...newRow);
        }
        return moved;
    }
    function moveUp() {
        ...
        for(let i = 0; i < width; i++){
            ...
            for(let j = 0; j < width; j++)board[i + j * width] = newColumn[j];
        }
        return moved;
    }
    function moveDown() {
    ...
    function control(e) {
        ...
        let moved = false;
        if (e.keyCode === 37 || e.key === "ArrowLeft") moved = moveLeft();
        else if (e.keyCode === 38 || e.key === "ArrowUp") moved = moveUp();
        else if (e.keyCode === 39 || e.key === "ArrowRight") moved = moveRight();
        else if (e.keyCode === 40 || e.key === "ArrowDown") moved = moveDown();
        if (moved) {
            addNumber();
            updateDisplay();
            checkGameOver();
    ...
    function checkGameOver() {
        ...
        for(let i = 0; i < board.length; i++){
            // Horizontal
            if (i % width < width - 1 && board[i] === board[i + 1]) return;
            // Vertical
            if (i < 12 && board[i] === board[i + width]) return;
        }
        showLose();
    }
    function showWin() {
